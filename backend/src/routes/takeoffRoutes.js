...
router.put("/project/:projectId/file/:fileId", (req, res) => {
  try {
    ...
    const body = req.body || {};
    // Accept either:
    //  A) { unitMode, scaleMmPerPx, data: {...} }   (preferred)
    //  B) raw takeoff object {...}                  (legacy frontend)
    const unitMode = body.unitMode || body.unit_mode || "imperial";
    // Support multiple spellings
    const scaleMmPerPx = body.scaleMmPerPx ?? body.scale_mm_per_px ?? body.unitsMmPerPx ?? body.units_mm_per_px;
    ...
    if (!existing) {
      const info = db.prepare(
        ...
        userId,
        unitMode || "imperial",
        Number((Number.isFinite(Number(scaleMmPerPx)) ? Number(scaleMmPerPx) : 1.0)),
        JSON.stringify(data || {}),
        now,
        now
      );
